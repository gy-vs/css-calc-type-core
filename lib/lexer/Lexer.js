import { SyntaxReferenceError, SyntaxMatchError, SyntaxMathError } from './error.js';
...
import checkMathExpressions from './math-check.js';
...
function matchSyntax(lexer, syntax, value, useCssWideKeywords) {
    ...
    // Type checking for calc()/min()/max()/clamp() expressions, which the
    // generic <length>/<angle>/... matchers accept as a balanced shell
    // without validating their contents. Values with var() are skipped above.
    //
    // Property and at-rule descriptor values are checked against their own
    // syntax (percentages resolve per property/descriptor); at-rule preludes
    // and anonymous type matches have no single value context, so percentages
    // are treated leniently there.
    const mathContextType = syntax.type === 'Property' || syntax.type === 'AtruleDescriptor'
        ? syntax.type
        : null;
    const mathError = checkMathExpressions(
        lexer,
        tokens,
        result.match,
        mathContextType ? { type: mathContextType, syntax: syntax.syntax } : null
    );

    if (mathError) {
        return buildMatchResult(
            null,
            new SyntaxMathError(mathError.message, syntax.syntax, value, result, mathError),
            result.iterations
        );
    }
