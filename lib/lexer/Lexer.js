...
function matchSyntax(lexer, syntax, value, useCssWideKeywords) {
    ...
    if (!useCssWideKeywords || !result.match) {
        ...
        if (!result.match) {
            const calcError = getCalcError();
            const calcErrorTokenIndex = calcError !== null
                ? tokens.indexOf(calcError.token)
                : -1;

            // when a mismatch is caused by an invalid calc()/min()/max()/clamp()
            // expression, report the expression error pointing to a problem
            // sub-expression instead of a generic mismatch; the expression error
            // is considered a cause when matching stalled at or after its location,
            // otherwise a regular mismatch is more relevant
            if (calcErrorTokenIndex !== -1 && calcErrorTokenIndex >= result.longestMatch) {
                return buildMatchResult(
                    null,
                    new SyntaxMatchError(
                        calcError.message,
                        syntax.syntax,
                        value,
                        { ...result, longestMatch: calcErrorTokenIndex }
                    ),
                    result.iterations
                );
            }
...
// A percentage in a math expression resolves to a type depending on a context
// where the expression is used (e.g. to a length for width, to a number for opacity,
// and it's not allowed at all for some properties). To check a percentage is allowed,
// replace every top level calc expression containing a percentage with a single
// percentage token and check the value still matches – the replaced value matches
// only when a percentage is acceptable in that position.
function checkCalcPercentageContext(lexer, syntax, value) {
    const tokens = prepareTokens(value, lexer.syntax);
    const ranges = findPercentageCalcRanges(tokens);

    for (const { start, end } of ranges) {
        const probeTokens = [
            ...tokens.slice(0, start),
            { type: Percentage, value: '1%', node: null },
            ...tokens.slice(end + 1)
        ];

        if (!matchAsTree(probeTokens, syntax.match, lexer).match) {
            return new SyntaxMatchError(
                'Percentage is not allowed in this context',
                syntax.syntax,
                value,
                { tokens, longestMatch: start }
            );
        }
    }
...
export class Lexer {
    ...
    matchProperty(propertyName, value) {
        ...
        const result = matchSyntax(this, property, value, true);

        // check a percentage in calc()/min()/max()/clamp() expressions
        // is allowed in a position where the expression is used
        if (result.matched !== null) {
            const calcError = checkCalcPercentageContext(this, property, value);

            if (calcError !== null) {
                return buildMatchResult(null, calcError);
            }
        }
