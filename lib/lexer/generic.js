...
import * as defaultUnits from './units.js';
import {
    checkCalcExpression,
    createUnitTypeMap,
    isCalcFunctionName,
...
//
// calc(), min(), max() and clamp() expressions are additionally checked for their
// type correctness (https://drafts.csswg.org/css-values-4/#calc-type-checking):
// every operand gets a type (a dimension group, a percentage or a number), operators
// combine operand types by the math function rules, and a resulting type should be
// compatible with expectedType (a generic type the expression is matched for).
// Expressions that can't be fully analyzed on this stage (e.g. containing var()
// or unknown functions) are passed through as is.
function math(next, functionNames, expectedType, unitTypes) {
    return function(token, getNextToken, opts) {
        ...
        if (token.type === FunctionToken && eqStrAny(token.value, functionNames)) {
            const length = consumeFunction(token, getNextToken);

            if (expectedType !== null && isCalcFunctionName(token.value)) {
                const tokens = new Array(length);

                for (let i = 0; i < length; i++) {
                    tokens[i] = getNextToken(i);
                }

                const { error } = checkCalcExpression(tokens, expectedType, unitTypes);

                if (error) {
                    recordCalcError(error);
                    return 0;
                }
            }
...
function createProductionTypes(unitTypes) {
    return {
        // token type aliases
        ...
        // percentage
        'percentage': math(percentage, percentageFunctionNames, 'percentage', unitTypes),

        // numeric
        'zero': zero(),
        'number': math(number, numberFunctionNames, 'number', unitTypes),
        'integer': math(integer, numberFunctionNames, 'integer', unitTypes),
...
export const productionTypes = createProductionTypes(createUnitTypeMap(defaultUnits));
...
export function createDemensionTypes(units, unitTypes = createUnitTypeMap(units || {})) {
    const {
        angle,
        decibel,
        frequency,
    ...
    return {
        'dimension': math(dimension(null), dimensionFunctionNames, 'dimension', unitTypes),
        'angle': math(dimension(angle), dimensionFunctionNames, 'angle', unitTypes),
        'decibel': math(dimension(decibel), dimensionFunctionNames, 'decibel', unitTypes),
        'frequency': math(dimension(frequency), dimensionFunctionNames, 'frequency', unitTypes),
        'flex': math(dimension(flex), dimensionFunctionNames, 'flex', unitTypes),
        'length': math(zero(dimension(length)), dimensionFunctionNames, 'length', unitTypes),
        'resolution': math(dimension(resolution), dimensionFunctionNames, 'resolution', unitTypes),
        'semitones': math(dimension(semitones), dimensionFunctionNames, 'semitones', unitTypes),
        'time': math(dimension(time), dimensionFunctionNames, 'time', unitTypes)
    };
}
...
export function createGenericTypes(units) {
    const unitTypes = createUnitTypeMap(units || {});

    return {
        ...tokenTypes,
        ...createProductionTypes(unitTypes),
        ...createDemensionTypes(units, unitTypes),
